--------------------------------------------------------------------------------
-- Steps to Call Interzoid's Get Business Info API
...
CREATE OR REPLACE FUNCTION YOUR_DB.PUBLIC.get_business_info(search_term STRING)
RETURNS VARIANT
LANGUAGE PYTHON
...
def get_info(search_term):
    ...
    # Endpoint for Business Info
    url = "https://api.interzoid.com/getbusinessinfo"
    
    # Parameters for Business Info
    params = {
        "license": license_key,
        "lookup": search_term
    }
    ...
    try:
        # TIMEOUT INCREASED TO 60 SECONDS HERE
...
-- Test 1: Lookup by Company Name
SELECT YOUR_DB.PUBLIC.get_business_info('Cisco');