...
CREATE OR REPLACE FUNCTION YOUR_DB.PUBLIC.get_email_score(email_address STRING)
RETURNS VARIANT
LANGUAGE PYTHON
...
def get_score(email_address):
    ...
    # Endpoint for Email Trust Score (Get Email Info)
...
GRANT USAGE ON FUNCTION YOUR_DB.PUBLIC.get_email_score(STRING) TO ROLE SYSADMIN;

--------------------------------------------------------------------------------
-- 6. TEST & EXECUTION
--------------------------------------------------------------------------------

-- Test 1: Check a valid email
SELECT YOUR_DB.PUBLIC.get_email_score('winston.smith@gmail.com');

-- Test 2: Check a disposable/temporary email (often returns lower score)
-- SELECT YOUR_DB.PUBLIC.get_email_score('temp1234@mailinator.com');