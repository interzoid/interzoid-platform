...
// Simple data class to hold the input address and its similarity key (SimKey)
data class Record(
    val input: String,
    val simKey: String
)
...
fun main() {
    ...
    inputFile.forEachLine { line ->
        ...
        // Skip blank lines
        if (address.isEmpty()) return@forEachLine

        val simKey = getAddressSimKey(address)

        // Skip if no SimKey returned
        if (simKey.isNullOrEmpty()) return@forEachLine

        records.add(Record(input = address, simKey = simKey))
    }
...
/**
 * Calls Interzoid's getaddressmatchadvanced API for a single street address
 * and returns the similarity key (SimKey) as a String.
 * Returns null or empty if there is an error or no SimKey.
 */
fun getAddressSimKey(address: String): String? {
    ...
    return try {
        ...
        println("Error calling API for \"$address\": ${e.message}")
        null
    }
}
