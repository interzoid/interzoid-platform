...
def call_address_match_api(address: str) -> str:
    """
    ...
    # URL-encode the address so it is safe to send as a URL parameter
    ...
    try:
        ...
        print(f"Error calling API for '{address}': {e}")
        return ""

    try:
        data = json.loads(body)
    except json.JSONDecodeError:
        print(f"Error parsing JSON for '{address}'. Raw response was:\n{body}\n")
        return ""
...
def main():
    # Each record holds the original address and its SimKey
    records = []
    ...
    try:
        with open(INPUT_FILE_NAME, "r", encoding="utf-8") as f:
            for line in f:
                ...
                simkey = call_address_match_api(address)

                # Skip if no SimKey returned
                if not simkey:
                    continue

                records.append({"input": address, "simkey": simkey})
    except OSError as e:
