...
//
// Reads street addresses from sample-input-file.txt, calls Interzoid's
// getaddressmatchadvanced API to generate similarity keys (SimKeys),
// then prints clusters of addresses that share the same SimKey.
//
...
function callAddressMatchAPI(address) {
  return new Promise((resolve) => {
    ...
    https
      .get(apiURL, (res) => {
        ...
        res.on("end", () => {
          try {
            ...
            console.error(
              `Error parsing JSON for "${address}": ${err.message}`
            );
      ...
      .on("error", (err) => {
        console.error(`Error calling API for "${address}": ${err.message}`);
        resolve("");
      });
  });
}
...
async function main() {
  ...
  // Process each non-empty line as a street address
  for (const line of lines) {
    const address = line.trim();
    if (!address) continue;

    const simKey = await callAddressMatchAPI(address);

    if (!simKey) continue;

    records.push({ input: address, simKey });
  }
